...
// const BASE_API_URL = "https://inventoryapi.nagarkaryavalinew.com/";
...
router.post("/:encodedEndpoint", async (req, res) => {
  ...
  try {
    const endpoint = Buffer.from(req.params.encodedEndpoint, "base64").toString("utf-8");
    ...
    if (!response.ok) {
      ...
      return res.status(500).json({ error: `Backend API error: ${response.status}` });
    }
    ...
    if (result.OUT_ERRORMSG && result.OUT_ERRORMSG.toLowerCase().includes("invalid")) {
      logger.error(`❌ Application-level error from ${endpoint}: ${result.OUT_ERRORMSG}`);
    }
...
router.get("/:encodedEndpoint", async (req, res) => {
  ...
  try {
    ...
    if (!payload) {
      ...
      return res.status(400).json({ error: "Missing encrypted payload in query" });
    }
